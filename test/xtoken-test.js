  ...
  describe("Xtoken", function () {
    ...
    describe("XToken manipulations", () => {
      ...
      it('Should add a token',async () => {
          const {xlock_contract, pricefeed_dai} = await loadFixture(deployTokenFixture);
    ...
    describe("BUY, LOCK, REWARD", () => {
      ...
      it("'BUY' works properly", async ()=> {
        const {xtoken_contract} = await loadFixture(deployTokenFixture);
      ...
      it("Lock and Reward should work properly", async () => {
        ...
        let y = 5
        while (y) {
          await xlock_contract.connect(coin_whale).deposit(DAI, 250000000000000, 1722690841, [[2, 100]], coin_whale.address, false);
